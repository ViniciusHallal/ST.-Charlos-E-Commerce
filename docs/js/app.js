...
let categoriaAtiva = 'Todas'; // filtro selecionado na página de produtos
...
/** Usado pelos atalhos de categoria da Home: já abre "Produtos" filtrado. */
...
function criarCardProduto(produto) {
  const info = getCategoriaInfo(produto.tipo);
  const card = document.createElement('div');
  card.className = 'card';
  card.innerHTML = `
    <div class="card-top">
      <div class="card-icon" style="background:${info.cor}1A; color:${info.cor}">${info.icone}</div>
      ${produto.receita ? '<span class="card-badge-receita">Requer receita</span>' : ''}
    </div>
    <div class="card-tag">${info.label}</div>
    <h3 class="card-title">${produto.nome}</h3>
    <div class="card-price">${formatBRL(produto.preco)}</div>
    <button class="btn-add" type="button" aria-label="Adicionar ${produto.nome} ao carrinho">Adicionar ao carrinho</button>
  `;
...
/** Chips de filtro ("Todas" + uma por categoria) no topo da página de produtos. */
function renderChipsCategorias() {
  const container = document.getElementById('chips-categorias');
  container.innerHTML = '';

  const criarChip = (tipo, label, icone) => {
    const chip = document.createElement('button');
    chip.type = 'button';
    chip.className = 'chip-categoria' + (categoriaAtiva === tipo ? ' ativo' : '');
    chip.innerHTML = icone ? `<span>${icone}</span> ${label}` : label;
  ...
  criarChip('Todas', 'Todas');
  getTipos().forEach((tipo) => {
    const info = getCategoriaInfo(tipo);
    criarChip(tipo, info.label, info.icone);
  });
}
...
function renderProdutos() {
  ...
  const tipos = categoriaAtiva === 'Todas' ? getTipos() : [categoriaAtiva];
  ...
  tipos.forEach((tipo) => {
    ...
    cabecalho.className = 'tipo-header';
    cabecalho.innerHTML = `
      <span class="card-icon" style="background:${info.cor}1A; color:${info.cor}">${info.icone}</span>
      <h2 class="section-title" style="margin-bottom:0">${info.label}</h2>
    `;
...
function renderPedidos() {
  ...
  if (!usuario) {
    container.innerHTML = `
      ...
      <button type="button" class="btn-login" style="max-width:220px;margin:0 auto;display:block;" onclick="navigate('login')">Ir para login</button>
    `;
    return;
  }
...
function renderFooterCategorias() {
  ...
  getTipos().forEach((tipo) => {
    ...
    btn.type = 'button';
    btn.className = 'footer-cat-link';
    btn.textContent = info.label;
    btn.addEventListener('click', () => irParaCategoria(tipo));
